...
def draw_board():
    ...
    for i in range(BOARD_SIZE):
        for j in range(BOARD_SIZE):
            ...
            # если это не нужная нам клетка, её следует оставить пустой
            if index != str(EMPTY_SQUARE):
                ...
                c.create_text(j * SQUARE_SIZE + SQUARE_SIZE / 2,
                              ...
                              text=index,
                              font="Verdana {} ".format(int(SQUARE_SIZE / 4)),
                              fill='#FFFFFF')


def show_victory_plate():
    ''' Функция, которая поздравляет с победой! '''
    ...
    c.create_text(SQUARE_SIZE * BOARD_SIZE / 2, SQUARE_SIZE * BOARD_SIZE / 1.9,
                  text="ПОБЕДА!", font="Verdana {} bold".format(int(10 * BOARD_SIZE)), fill='#C9BD7F')
